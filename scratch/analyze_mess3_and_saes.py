...
active_latents = {}
def gen_active_pcts(inds):
    max_count = 0
    for k in inds.keys():
        if inds[k][0] > max_count:
            max_count = inds[k][0]
    for k in inds.keys():
        inds[k][0] = inds[k][0] / max_count
    return inds
    ...
    for site_name, site_data in metrics_summary.items():
        ...
        for sae_type in ("top_k", "vanilla"):
            ...
            for name, rec in group.items():
                ...
                if inds:
                    active_latents[(site_name, "sequence", sae_type, name)] = gen_active_pcts(inds)
        # Beliefs vanilla group
        beliefs = site_data.get("beliefs", {})
        for name, rec in beliefs.items():
            inds = rec.get("active_latents_last_quarter", [])
            if inds:
                active_latents[(site_name, "beliefs", "vanilla", name)] = gen_active_pcts(inds)
...
if metrics_summary is not None:
    if args.print_last50_loss:
        generate_last50_loss_plots(metrics_summary, output_dir)
    generate_l0_bar_plots(metrics_summary, output_dir)
    generate_vanilla_l0_average_line_plots(metrics_summary, output_dir)
else:
...
print(f"{token_labels.shape=}")
for k in residual_streams.keys():
    print(f"{k}: {residual_streams[k].shape=}")
    print(f"{k}: {activations_flat[k].shape=}")
...
for key, val in active_latents.items():
    if isinstance(val, dict):
        ...
        try:
            ...
            count = 0
    active_latent_counts[key] = count
    alc_cnts = []
    for ind in val.keys():
        if val[ind][0] > 0.01:
            alc_cnts.append(int(ind))
    alc_cut[key] = len(alc_cnts)

# for key, val in alc_cut.items():
#     print(f"{key}, {val=}")
...
#%%
##### Plot EPDFs for all SAEs #####
###################################

def get_act_lats_sorted(act_lats):
    alats = []
    for k in act_lats.keys():
        alats.append((int(k), act_lats[k][0], act_lats[k][1]))
    alats.sort(key=lambda x: x[1], reverse=True)
    return alats


active_latents_cut = {}
seq_lambdas = [0.01, 0.025, 0.05, 0.1, 0.11, 0.115, 0.13]
seq_figs_dict = {}
min_latent_pct = 0.01

# for layer in [0]:
for layer in range(4):
    seq_figs_dict[f'layer_{layer}'] = {}
    seq_figs_dict[f'layer_{layer}']['top_k'] = {}
    seq_figs_dict[f'layer_{layer}']['vanilla'] = {}
    # for k in [5]:
    for k in range(1,8):
        print(f"layer {layer}, k {k}")
        seq_figs_dict[f'layer_{layer}']['top_k'][f'k{k}'] = {}
        alats = get_act_lats_sorted(active_latents[(f'layer_{layer}', 'sequence', 'top_k', f'k{k}')])
        fig_epdfs = [sequence_epdfs[f'layer_{layer}']['top_k'][f'k{k}'][int(alt[0])] \
                       for alt in alats if alt[1] > min_latent_pct and \
                                           alt[0] in sequence_epdfs[f'layer_{layer}']['top_k'][f'k{k}'] and \
                                           sequence_epdfs[f'layer_{layer}']['top_k'][f'k{k}'][int(alt[0])].coords.shape[0] > 5]
        triangle_vertices = get_global_triangle_vertices(fig_epdfs)
        epdf_fig = plot_epdfs(fig_epdfs, mode="transparency",
                              active_latents=active_latents[(f'layer_{layer}', 'sequence', 'top_k', f'k{k}')],
                              triangle_vertices=triangle_vertices)

        epdf_fig.write_image(os.path.join(output_dir, f"epdf_fig_layer_{layer}_topk_k_{k}.png"))
        #epdf_fig.show()
        seq_figs_dict[f'layer_{layer}']['top_k'][f'k{k}']["all"] = epdf_fig
        print(f"{len(fig_epdfs)=}")
        if len(fig_epdfs) > 4:
            for ii, epdf in enumerate(fig_epdfs):
                print(f"Separately plotting {epdf}")
                epdf_fig = plot_epdfs([epdf], mode="transparency", grid_size=80, triangle_vertices=triangle_vertices,
                                      active_latents=active_latents[(f'layer_{layer}', 'sequence', 'top_k', f'k{k}')],
                                      color_start_ind=ii)
                epdf_fig.write_image(os.path.join(output_dir, f"epdf_fig_layer_{layer}_topk_k_{k}_latent_{int(epdf.latent_idx)}.png"))
                seq_figs_dict[f'layer_{layer}']['top_k'][f'k{k}'][int(epdf.latent_idx)] = epdf_fig
    #for l in [0.01]:
    for l in seq_lambdas:
        print(f"\n\nlayer {layer}, lambda {l}")
        seq_figs_dict[f'layer_{layer}']['vanilla'][f'lambda_{l}'] = {}
        alats = get_act_lats_sorted(active_latents[(f'layer_{layer}', 'sequence', 'vanilla', f'lambda_{l}')])
        active_latents_cut[(f'layer_{layer}', 'sequence', 'vanilla', f'lambda_{l}')] = {alt[0]: (alt[1], alt[2]) for alt in alats if alt[1] > min_latent_pct}
        fig_epdfs = [sequence_epdfs[f'layer_{layer}']['l1'][f'lambda_{l}'][int(alt[0])] \
                       for alt in alats if alt[1] > min_latent_pct and \
                                           alt[0] in sequence_epdfs[f'layer_{layer}']['l1'][f'lambda_{l}'] and \
                                           sequence_epdfs[f'layer_{layer}']['l1'][f'lambda_{l}'][int(alt[0])].coords.shape[0] > 5]
        triangle_vertices = get_global_triangle_vertices(fig_epdfs)
        epdf_fig = plot_epdfs(fig_epdfs, mode="transparency",
                              active_latents=active_latents[(f'layer_{layer}', 'sequence', 'vanilla', f'lambda_{l}')],
                              triangle_vertices=triangle_vertices)
        epdf_fig.write_image(os.path.join(output_dir, f"epdf_fig_layer_{layer}_vanilla_lambda_{l}.png"))
        #epdf_fig.show()
        seq_figs_dict[f'layer_{layer}']['vanilla'][f'lambda_{l}']["all"] = epdf_fig
        if len(fig_epdfs) > 4:
            for ii, epdf in enumerate(fig_epdfs):
                print(f"Separately plotting {epdf}")
                epdf_fig = plot_epdfs([epdf], mode="transparency", grid_size=80, triangle_vertices=triangle_vertices,
                                      active_latents=active_latents[(f'layer_{layer}', 'sequence', 'vanilla', f'lambda_{l}')],
                                      color_start_ind=ii)
                epdf_fig.write_image(os.path.join(output_dir, f"epdf_fig_layer_{layer}_vanilla_lambda_{l}_latent_{int(epdf.latent_idx)}.png"))
                seq_figs_dict[f'layer_{layer}']['vanilla'][f'lambda_{l}'][int(epdf.latent_idx)] = epdf_fig


#%%
print(f"{loaded_saes.keys()=}")
print(f"{loaded_saes['layer_1'].keys()=}")
print(f"{loaded_saes['layer_1'][None, 0.025].keys()=}")
print(f"{loaded_saes['layer_1'][None, 0.025]['state_dict'].keys()=}")
print(f"{loaded_saes['layer_1'][None, 0.025]['cfg'].keys()=}")


#%%
##### Plot Live SAE Latent Decoder Directions Via Residual Stream PCAs #####
#############################################################################
import importlib
mess3_saes_pca = importlib.import_module("mess3_saes_pca")
importlib.reload(mess3_saes_pca)
from mess3_saes_pca import (
    project_decoder_directions,
    plot_decoder_projections,
)

# Prepare inputs: sae_entries and latent_indices, then project and plot
...
for site_name, sae_dict in loaded_saes.items():
    for (k, lmbda), sae_ckpt in sae_dict.items():
        # Decide SAE class and type/param string
        if k is not None:
            sae_class = TopKSAE
            sae_type = "top_k"
            param = f"k{k}"
            active_key = (site_name, "sequence", "top_k", param)
            epdf_dict = sequence_epdfs.get(site_name, {}).get('top_k', {}).get(param, {})
        else:
            sae_class = VanillaSAE
            sae_type = "vanilla"
            param = f"lambda_{lmbda}"
            active_key = (site_name, "sequence", "vanilla", param)
            epdf_dict = sequence_epdfs.get(site_name, {}).get('l1', {}).get(param, {})

        # Rebuild SAE object
        sae = sae_class(sae_ckpt["cfg"])
        sae.load_state_dict(sae_ckpt["state_dict"])  # type: ignore
        sae.to(device).eval()

        # Collect active latent indices for this SAE, if available
        inds = active_latents.get(active_key, None)
        if isinstance(inds, dict):
            # keys may be str; convert to int; optionally filter by activity fraction
            try:
                idxs_sorted = sorted(
                    [(int(k_), v_[0]) for k_, v_ in inds.items()], key=lambda x: x[1], reverse=True
                )
            except Exception:
                idxs_sorted = [(int(k_), 1.0) for k_ in inds.keys()]
            # Now filter by both activity fraction and coords count
            idxs = []
            for i, frac in idxs_sorted:
                if frac > 0.01:
                    epdf = epdf_dict.get(i, None)
                    if epdf is not None and hasattr(epdf, "coords") and getattr(epdf, "coords", None) is not None:
                        if getattr(epdf, "coords").shape[0] >= 20:
                            idxs.append(i)
            # If none pass, fallback to just activity
            if not idxs:
                for i, _ in idxs_sorted:
                    epdf = epdf_dict.get(i, None)
                    if epdf is not None and hasattr(epdf, "coords") and getattr(epdf, "coords", None) is not None:
                        if getattr(epdf, "coords").shape[0] >= 20:
                            idxs.append(i)
        elif inds is not None:
            # For non-dict inds, just filter by coords count
            idxs = []
            for i in inds:
                i_int = int(i)
                epdf = epdf_dict.get(i_int, None)
                if epdf is not None and hasattr(epdf, "coords") and getattr(epdf, "coords", None) is not None:
                    if getattr(epdf, "coords").shape[0] >= 20:
                        idxs.append(i_int)
        else:
            # Fallback to plot nothing if we don't know which latents are active
            idxs = []
        ...
        latent_indices[(site_name, sae_type, param)] = idxs


#%%
# Choose projection dimensionality compatible with available PCAs
if pcas:
    try:
        min_components = min(
            (p.components_.shape[0] if hasattr(p, 'components_') else (p.n_components if hasattr(p, 'n_components') else 0))
            for p in pcas.values()
        )
    except Exception:
        min_components = 2
else:
    min_components = 2
proj_dims = 3 if min_components >= 3 else 2

projections = project_decoder_directions(
    sae_entries=sae_entries,
    pcas=pcas,
...
#%%
# Plot per (site, sae_type, param) by themselves
for site, type_map in projections.items():
    for sae_type, param_map in type_map.items():
        for param, lat_map in param_map.items():
            # Filter which SAEs to plot with activations overlay
            allowed_topk = {"k2", "k3", "k4"}
            allowed_lambda = {"lambda_0.075", "lambda_0.1", "lambda_0.11", "lambda_0.115"}
            if sae_type == "top_k" and param not in allowed_topk:
                continue
            if sae_type == "vanilla" and param not in allowed_lambda:
                continue
            selection = [(site, sae_type, param, li) for li in lat_map.keys()]
            if not selection:
                continue
            title = f"Decoder Directions — {site} / {sae_type} / {param}"
            # Build activity rates for legend (use sequence active_latents)
            activity_rates = {}
            try:
                active_dict = active_latents.get((site, "sequence", sae_type, param), {})
                # active_dict maps latent_idx -> [fraction, sum_magnitudes]
                for li in lat_map.keys():
                    v = None
                    if isinstance(active_dict, dict):
                        v = active_dict.get(str(li)) if str(li) in active_dict else active_dict.get(int(li))
                    if isinstance(v, (list, tuple)) and len(v) > 0:
                        activity_rates[(site, sae_type, param, int(li))] = float(v[0])
            except Exception:
                pass

            fig = plot_decoder_projections(
                projections,
                dims=proj_dims,
                selection=selection,
                title=title,
            ...
            if proj_dims == 3:
                fig.update_layout(scene_camera=dict(eye=dict(x=0.9, y=1.3, z=2.2)))
            out_path = os.path.join(
                output_dir,
                f"decoder_dirs_{site}_{sae_type}_{param}_pc{proj_dims}.png"
            )
            try:
                ...
                print(f"Failed to save decoder directions for {site}/{sae_type}/{param}: {e}")
        break
    break


#%%
# Plot per (site, sae_type, param) with activation projections and overlay activation point clouds in 3D using pca_coords
for site, type_map in projections.items():
    for sae_type, param_map in type_map.items():
        for param, lat_map in param_map.items():
            allowed_topk = {"k2", "k3", "k4"}
            ...
            # Determine a reasonable scale so cones and point cloud are comparable
            scale_factor = 1.0
            coords3 = None
            if proj_dims == 3 and isinstance(pca_coords.get(site, None), np.ndarray):
                site_coords = pca_coords[site]
                if site_coords.ndim == 2 and site_coords.shape[0] > 0 and site_coords.shape[1] >= 3:
                    coords3 = site_coords[:, :3]
                    # Compute a robust radius of the cloud
                    norms = np.linalg.norm(coords3, axis=1)
                    if norms.size > 0:
                        cloud_scale = np.percentile(norms, 90)
                        # Compute vector norms for current selection
                        vec_norms = []
                        for _, _, _, li in selection:
                            try:
                                v = projections[site][sae_type][param][int(li)]
                                if v.shape[0] >= 3:
                                    vec_norms.append(float(np.linalg.norm(v[:3])))
                            except Exception:
                                continue
                        mean_vec = np.mean(vec_norms) if len(vec_norms) > 0 else 1.0
                        if mean_vec > 0:
                            scale_factor = cloud_scale / (mean_vec + 1e-9)

            # Plot points first (for scale and layering), then add decoder directions
            fig = go.Figure()
            if proj_dims == 3 and coords3 is not None:
                # Subsample for visualization if too many points
                if coords3.shape[0] > 9000:
                    idxs_all = np.random.choice(coords3.shape[0], 9000, replace=False)
                    coords3 = coords3[idxs_all]
                    tl = token_labels[idxs_all]
                else:
                    tl = token_labels
                ...
                for token_id in range(3):
                    ...
                    fig.add_trace(
                        go.Scatter3d(
                            ...
                            showlegend=True if token_id == 0 else False,
                        )
                    )

            vec_fig = plot_decoder_projections(
                projections,
                dims=proj_dims,
                selection=selection,
                title=title,
            ...
            # Append decoder direction traces to the same figure
            fig.add_traces(vec_fig.data)
            # Apply layout (keep decoder layout titles/axes)
            fig.update_layout(
                title=title,
                scene=dict(
                    ...
                    aspectmode="cube",
                ) if proj_dims == 3 else None,
                scene_camera=dict(eye=dict(x=0.9, y=1.3, z=2.2)) if proj_dims == 3 else None,
                height=700 if proj_dims == 2 else 700,
                width=800 if proj_dims == 2 else 900,
                legend=dict(font=dict(color="black")),
            )

            out_path = os.path.join(
                output_dir,
                f"decoder_dirs_{site}_{sae_type}_{param}_pc{proj_dims}_w_activations.png"
            )
            try:
                fig.write_image(out_path)
                fig.show()
...
#%%
for k in sequence_epdfs['layer_1']['l1']['lambda_0.025'].keys():
    print(f"latent {k} has coord shape: {sequence_epdfs['layer_1']['l1']['lambda_0.025'][k].coords.shape}")
...
for site_name, sae_dict in loaded_saes.items():
    for (k, lmbda), sae_ckpt in sae_dict.items():
        ...
        # Save histograms
        layer_report_dir = os.path.join(output_dir, f"{site_name}_k{k}_lambda{lmbda}")
...
# # Loop over all available PCA projections in pca_coords (now a dict keyed by site_name)
# colors = ['red', 'green', 'blue']
# token_names = ['Token 0', 'Token 1', 'Token 2']

# for site_name, coords in pca_coords.items():
#     # Check if coords has at least 3 components for 3D plot
#     if coords.shape[1] < 3:
#         print(f"Skipping {site_name}: less than 3 PCA components.Shape is {coords.shape}")
#         continue
...
# fig.update_layout(
#         title=f'3D PCA of Residual Stream ({site_name})',
#         scene=dict(
#         xaxis_title='PC1',
#         yaxis_title='PC2',
#             zaxis_title='PC3'
