...
# Full Real Data Pipeline with Co-occurrence Clustering (512 clusters)
#
...
#
# Run this on one GPU while run_full_real_data_pipeline_cooc_768.sh runs on another.
...
COOC_CACHE="${OUTPUT_BASE}/cooc_stats_512.npz"

echo "============================================================"
echo "CO-OCCURRENCE CLUSTERING PIPELINE (512 clusters)"
echo "============================================================"
...
echo ""
echo "============================================================"
echo "STAGE 1A: Running pipeline with PHI similarity (512 clusters)"
echo "============================================================"
...
python -u real_data_tests/analyze_real_saes.py \
    ...
    --output_dir "${OUTPUT_BASE}/phi_512" \
    --n_clusters_list 512 \
...
echo ""
echo "PHI clustering + AANet fitting complete for 512 clusters"
echo ""
...
echo ""
echo "============================================================"
echo "STAGE 1B: Running pipeline with MUTUAL INFO similarity (512 clusters)"
echo "============================================================"
...
python -u real_data_tests/analyze_real_saes.py \
    ...
    --sae_release "gemma-scope-9b-pt-res" \
    --sae_id "layer_20/width_16k/average_l0_68" \
    --output_dir "${OUTPUT_BASE}/mutual_info_512" \
    --n_clusters_list 512 \
    --latent_activity_threshold 0 \
    --activity_batch_size 32 \
    --activity_batches 4096 \
...
echo ""
echo "MUTUAL INFO clustering + AANet fitting complete for 512 clusters"
echo ""
...
echo ""
echo "============================================================"
echo "STAGE 2A: Collecting vertex samples for PHI clusters (512)"
echo "============================================================"

python -u real_data_tests/refit_selected_clusters.py \
    --skip_training \
    --stage1_models_dir "${OUTPUT_BASE}/phi_512" \
    --vertex_skip_docs 300000 \
    --n_clusters_list "512" \
    --csv_dir "${OUTPUT_BASE}/phi_512" \
    --save_dir "${OUTPUT_BASE}/phi_512_selected" \
    --model_name "gemma-2-9b" \
    --sae_release "gemma-scope-9b-pt-res" \
    --sae_id "layer_20/width_16k/average_l0_68" \
    --device "cuda" \
...
echo ""
echo "============================================================"
echo "STAGE 2B: Collecting vertex samples for MUTUAL INFO clusters (512)"
echo "============================================================"

python -u real_data_tests/refit_selected_clusters.py \
    --skip_training \
    --stage1_models_dir "${OUTPUT_BASE}/mutual_info_512" \
    --vertex_skip_docs 300000 \
    --n_clusters_list "512" \
    --csv_dir "${OUTPUT_BASE}/mutual_info_512" \
    --save_dir "${OUTPUT_BASE}/mutual_info_512_selected" \
    --model_name "gemma-2-9b" \
    --sae_release "gemma-scope-9b-pt-res" \
    --sae_id "layer_20/width_16k/average_l0_68" \
    --device "cuda" \
...
echo ""
echo "============================================================"
echo "PIPELINE COMPLETE (512 clusters)"
echo "============================================================"
echo ""
echo "Outputs:"
echo "  PHI clustering:        ${OUTPUT_BASE}/phi_512/"
echo "  PHI vertex samples:    ${OUTPUT_BASE}/phi_512_selected/"
echo "  MI clustering:         ${OUTPUT_BASE}/mutual_info_512/"
echo "  MI vertex samples:     ${OUTPUT_BASE}/mutual_info_512_selected/"
echo "  Co-occurrence cache:   ${COOC_CACHE}"
echo ""
echo "============================================================"
